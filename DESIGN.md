...
| 분류               | 토큰명                    | Hex 코드  | 용도                                                       |
| :----------------- | :------------------------ | :-------- | :--------------------------------------------------------- |
| **Brand**          | `--color-primary`         | `#00236f` | 메인 네이비, 브랜드 요소, 활성 텍스트, Secondary 버튼 보더 |
...
- **Display-LG:** `32px / 2rem`, Line-height: `1.25`, Bold (메인 페이지 제목)
- **Headline-SM:** `24px / 1.5rem`, Line-height: `1.3`, Bold (섹션 제목)
- **Title-MD:** `18px / 1.125rem`, Line-height: `1.4`, SemiBold (카드 제목, 문제 이름)
- **Body-MD:** `14px / 0.875rem`, Line-height: `1.6`, Regular (기본 본문, 테이블 데이터)
- **Label-SM:** `12px / 0.75rem`, Line-height: `1.4`, Medium (배지, 메타데이터, 캡션)
- **Code:** `13px / 0.8125rem`, Line-height: `1.6`, Monospace (코드 스니펫, 복잡도 지표)
...
| :-------------- | :----------------------------------------------------- | :------------------------------------------------- | :--------------------------- | :--------------------------------------------------- |
| **Accent CTA**  | BG: `--color-secondary`, Text: White                   | BG: `--color-secondary-hover`                      | Scale(0.96)                  | BG: `#d1d5db`, Text: White                           |
...
- **형태:** 필(Pill) 모양을 사용하며 `Radius: 999px`, `Padding: 4px 8px`를 적용한다.
- **타이포그래피:** `Label-SM`을 사용하며, 상태나 난이도를 짧은 텍스트로 명확하게 표시한다.
- **의미 전달:** 색상에만 의존하지 않고 텍스트 라벨 또는 아이콘(체크, 느낌표, 잠금 등)을 함께 사용한다.

#### 상태 배지 (Status Badges)

- **Completed / Success:** BG: `rgba(34, 197, 94, 0.1)`, Text: `--color-success`, Icon: 체크 아이콘.
- **Review Needed / Warning:** BG: `rgba(245, 158, 11, 0.12)`, Text: `--color-warning`, Icon: 느낌표 아이콘.
- **Failed / Danger:** BG: `rgba(239, 68, 68, 0.1)`, Text: `--color-danger`, Icon: 경고 아이콘.
- **Locked:** BG: `--color-surface-dim`, Text: `--color-text-muted`, Icon: 자물쇠 아이콘.

#### 난이도 배지 (Difficulty Badges)

- **Easy:** BG: `rgba(34, 197, 94, 0.1)`, Text: `--color-success`.
- **Medium:** BG: `rgba(245, 158, 11, 0.12)`, Text: `--color-warning`.
- **Hard:** BG: `rgba(239, 68, 68, 0.1)`, Text: `--color-danger`.

#### 사용 규칙

- 문제 난이도, 풀이 상태, 복습 여부는 반드시 배지로 표시한다.
- 동일한 행 또는 카드 안에서 배지가 여러 개 사용될 경우 `gap: 8px`을 적용한다.
- 모바일 카드에서는 문제 제목 아래 또는 메타데이터 영역에 배지를 배치한다.
...
### 6.6 입력 필드 및 필터 (Form Inputs & Selects)

- **Default:** BG: `--color-surface-dim`, Border: 1px solid `--color-border`, Radius: 4px.
- **Hover:** Border: 1px solid `--color-border-variant`.
- **Focus:** Border: 1px solid `--color-primary`, Box-shadow: `0 0 0 3px rgba(0, 35, 111, 0.1)`.
- **Placeholder:** Text: `--color-text-muted`, Font-size: `14px`.
- **Disabled:** BG: `#f1f5f9`, Text: `--color-text-muted`, Cursor: not-allowed.
